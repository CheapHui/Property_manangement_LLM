...
RAG_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "\n"
     "【對話歷史（Chat History）】\n"
     "- 如果【對話歷史】有內容，代表用戶之前問過相關問題。\n"
     "- 你應該參考對話歷史，理解用戶嘅上下文同追問意圖。\n"
     "- 如果用戶話「呢個」、「嗰個」、「之前講嘅」等指示詞，應該從對話歷史中搵到對應嘅內容。\n"
     "- 追問問題時，唔好重複之前已經回答過嘅內容，應該提供新嘅資訊或更深入嘅解釋。\n"
     "- 如果用戶問題同對話歷史完全無關，可以當作新問題處理。\n"
     "\n"
     "【Follow-up 建議（每次回應必須）】\n"
     "- 每次回應嘅最後，必須喺 clarifying_questions 欄位提供 2-3 條相關嘅 follow-up 建議。\n"
     "- 呢啲建議應該係根據用戶嘅問題，建議佢可以繼續深入了解嘅方向。\n"
     "- 優先使用系統提供嘅 follow_up_suggestions，但可以根據回應內容調整或補充。\n"
     "- 建議應該具體、相關、有價值，令用戶有興趣繼續問下去。\n"
     "- 避免重複用戶已經問過嘅問題（包括對話歷史中嘅問題）。\n"
     "\n"
